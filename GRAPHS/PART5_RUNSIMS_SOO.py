...
def executeSimulations(info):
    ...
    if generateParams == "manual":
        parameters = np.load(f"{outputFileDirectory}/initial/common/parameters.npy", allow_pickle=True).tolist()
    ...
    if not os.path.exists(f"{outputFileDirectory}/initial/common/FD_Curves_unsmooth.npy"):
        ...
        numberOfInitialSims = len(np.load(f"{outputFileDirectory}/initial/common/FD_Curves_unsmooth.npy", allow_pickle=True).tolist())
